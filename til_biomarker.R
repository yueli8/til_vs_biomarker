...
setwd("~/gse184053")
DF<-read.table("gse184053_total.txt", head=TRUE)
b<-aggregate(DF[, -c(1:2)], by=list(DF$EntrzID, DF$Name), mean)
write.table(b, file="gse184053_averageif.txt")

#pca 
setwd("~/gse184053")
gse184053<-read.table("gse184053_gene_name")
...
pca<-prcomp(t(gse184053))
plot(pca)
{plot(pca$x[,1],pca$x[,2])
  points(pca$x[1:3,1],pca$x[1:3,2], main="Top 2 PCs",col=2)
  points(pca$x[4:6,1],pca$x[4:6,2], main="Top 2 PCs",col=3)
  points(pca$x[7:9,1],pca$x[7:9,2], main="Top 2 PCs",col=4)
}
which(pca$x[,1]< -50)
which(pca$x[,2]< -50)
...
setwd("~/gse184053")
data4 <- read.table("gse184053_gene_name", header=T, row.names=1,sep="\t")
...
print(sample[1:4])# 获得样品分组信息
sample <- rownames(data_var3_forPCA)
group <- unlist(lapply(strsplit(sample, "_"), function(x) x[2]))
print(sample[1:4])
print(group[1:4])# 根据分组数目确定颜色变量
...
percentVar <- pca$sdev^2 / sum( pca$sdev^2)
print(str(pca))
autoplot(pca, data=data4_use_log2_label, colour="group") + xlab(paste0("PC1 (", round(percentVar[1]*100), "% variance)")) 
+ ylab(paste0("PC2 (", round(percentVar[2]*100), "% variance)")) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + theme(legend.position="right")
...
scatterplot3d(x=pc$PC1, y=pc$PC2, z=pc$PC3, pch=16, color=colors, xlab=paste0("PC1 (", round(percentVar[1]*100), "% variance)"), 
              ylab=paste0("PC2 (", round(percentVar[2]*100), "% variance)"), zlab=paste0("PC3 (", round(percentVar[3]*100), "% variance)"), angle=10)
legend("topright", legend=levels(as.factor(group)), col=colorl, pch=16, xpd=T, horiz=F, ncol=6)
...
setwd("~/gse184053")
cts<-read.table("p_p_vs_p_h.txt",head=TRUE) 
...
head(dds_norm)
write.csv(dds_norm,"p_p_vs_p_h_norm.csv")
dds2<-DESeq(dds,fitType = "local")
resultsNames(dds2)
res <- results(dds2)
res <- results(dds2, contrast=c("condition","p_p","p_h"))
resultsNames(dds2)
write.csv(res,"p_p_vs_p_h_deg_local.csv")
...
#volcano plot til_vs_p_p
setwd("~/gse184053")
cts<-read.table("til_vs_p_p.txt",head=TRUE) 
#setup DESeqDataSetFromMatrix
cts_round<-round(cts)
countData<-cts_round
condition <- factor(c(rep("til",3),rep("p_p",3)), levels = c("til","p_p"))
coldata<-data.frame(row.names=colnames(countData),condition)
condition
coldata
dds<-DESeqDataSetFromMatrix(countData=countData, colData=coldata, design=~condition)
...
#volcano plot til_vs_p_h
setwd("~/gse184053")
cts<-read.table("til_vs_p_h.txt",head=TRUE) 
#setup DESeqDataSetFromMatrix
cts_round<-round(cts)
countData<-cts_round
condition <- factor(c(rep("til",3),rep("p_h",3)), levels = c("til","p_h"))
coldata<-data.frame(row.names=colnames(countData),condition)
condition
coldata
dds<-DESeqDataSetFromMatrix(countData=countData, colData=coldata, design=~condition)
...
dds <- dds[keep,]
dim(dds)
dds$condition <- factor(dds$condition, levels = c("til","p_h"))
dds$condition <- relevel(dds$condition, ref = "p_h")
dds$condition <- droplevels(dds$condition)
dds <- estimateSizeFactors(dds)
dds_norm<-counts(dds,normalized=TRUE)
head(dds_norm)
write.csv(dds_norm,"til_vs_p_h_norm.csv")
dds2<-DESeq(dds,fitType = "local")
resultsNames(dds2)
res <- results(dds2)
res <- results(dds2, contrast=c("condition","til","p_h"))
resultsNames(dds2)
write.csv(res,"til_vs_p_h_deg.csv")

setwd("~/gse184053")
res <- read.csv("til_vs_p_h_deg.csv", header=TRUE)
head(res)
with(res, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot", xlim=c(-8,12)))
# Add colored points: red if pvalue<0.05, orange of log2FC>1, green if both)
with(subset(res, padj<.05 ), points(log2FoldChange, -log10(padj), pch=20, col="red"))
...
#pheatmap
mat<-read.table("OR_194.txt", head=TRUE, row.names = 1)
x<-as.matrix(mat)
x<-t(x)
...
mat<-read.table("AND_76.txt", head=TRUE, row.names = 1)
x<-as.matrix(mat)
x<-t(x)
