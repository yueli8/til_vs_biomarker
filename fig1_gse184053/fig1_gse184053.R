...
DF<-read.table("gse184053_total.txt",header = TRUE)
b<-aggregate(DF[, -c(1:2)], by=list(DF$EntrzID, DF$Name), mean)
write.table(b, file="gse184053_averageif.txt")

#pca 
setwd("~/gse184053/data/data_finally")
gse184053<-read.table("gse184053_gene_name",header=TRUE)
...
#volcano_plot
setwd("~/gse184053/data/data_finally")
res <- read.csv("til_vs_p_p_deg.txt", header=TRUE,sep="\t")
head(res)
with(res, plot(log2FoldChange, -log10(fdr), pch=20, main="Volcano plot", xlim=c(-10,12),col="grey"))
...
setwd("~/gse184053/data/data_finally")
res <- read.csv("til_vs_p_h_deg.txt", header=TRUE,sep="\t")
head(res)
with(res, plot(log2FoldChange, -log10(fdr), pch=20, main="Volcano plot", xlim=c(-6,12),col="grey"))
# Add colored points: red if pvalue<0.05, orange of log2FC>1, green if both)
#with(subset(res, fdr<.05 ), points(log2FoldChange, -log10(fdr), pch=20, col="red"))
#with(subset(res, abs(log2FoldChange)>1), points(log2FoldChange, -log10(fdr), pch=20, col="orange"))
with(subset(res, fdr<.05 & abs(log2FoldChange)>1), points(log2FoldChange, -log10(fdr), pch=20, col="blue"))
...
#pheatmap
mat<-read.table("OR_194.txt", head=TRUE, row.names = 1)#use the file of heatmap_194.txt
x<-as.matrix(mat)
x<-t(x)
